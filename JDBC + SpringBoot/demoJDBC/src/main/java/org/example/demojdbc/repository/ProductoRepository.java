...
public class ProductoRepository {
    ...
    public void insertar(Producto producto) {
        jdbcTemplate.update("INSERT INTO Productos (nombre, descripcion, precio, stock) VALUES (?, ?, ?, ?)",
    ...
    public Producto buscarPorId(int id) {
        List<Producto> resultados = jdbcTemplate.query("SELECT * FROM Productos WHERE id = ?", new ProductoMapper(), id);
        return resultados.isEmpty() ? null : resultados.get(0);
    }

    public Producto buscarPorNombre(String nombre) {
        List<Producto> resultados = jdbcTemplate.query("SELECT * FROM Productos WHERE nombre = ?", new ProductoMapper(), nombre);
    ...
    public void borrarPorNombre(String nombre) {
        jdbcTemplate.update("DELETE FROM Productos WHERE nombre = ?", nombre);
    }
